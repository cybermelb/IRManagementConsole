...
const RiskDashboardPage = () => {
  ...
  useEffect(() => {
    const handler = () => {
      try {
        const t = localStorage.getItem("timelineEvents");
        if (t) setTimelineEvents(JSON.parse(t));
        const r = localStorage.getItem("remediationTasks");
...
const IncidentBuilderPage = ({ assets = [] }) => {
  // Seed incidents: prefer localStorage, fall back to initialScenarios
  const [incidentLog, setIncidentLog] = useState(() => {
    try {
      const saved = localStorage.getItem("incidentLog");
      if (saved) return JSON.parse(saved);
      // fallback to pre-seeded scenarios if available
      return Array.isArray(initialScenarios) ? initialScenarios : [];
    } catch {
      return Array.isArray(initialScenarios) ? initialScenarios : [];
    }
  });
  ...
  // Timeline and remediation state
  const [timelineEvents, setTimelineEvents] = useState(() => {
    try {
      const saved = localStorage.getItem("timelineEvents");
      return saved ? JSON.parse(saved) : [];
  ...
  // Logged By roles from AdminDashboardPage
  ...
  // Keep roles in sync if Admin updates them while this page is open
  useEffect(() => {
    const handler = () => {
      try {
        const updated = localStorage.getItem("incidentUsers");
        if (updated) setUsers(JSON.parse(updated));
      } catch {}
    };
    window.addEventListener("storage", handler);
    return () => window.removeEventListener("storage", handler);
  ...
  const [newEvent, setNewEvent] = useState({
    ...
    phase: "Identification",
    remediation: "",
    user: users[0] || "SOC Analyst 1",
  });

  // Persist helpers
  const persistIncidentLog = (list) =>
    localStorage.setItem("incidentLog", JSON.stringify(list));
  const persistTimeline = (list) =>
    localStorage.setItem("timelineEvents", JSON.stringify(list));
  const persistTasks = (list) =>
    localStorage.setItem("remediationTasks", JSON.stringify(list));
  ...
  const handleIntakeSubmit = (e) => {
    ...
    const newEntry = {
      id: (incidentLog[0]?.id || 0) + 1, // simple id scheme: next after latest
    ...
    const updatedIncidents = [newEntry, ...incidentLog];
    setIncidentLog(updatedIncidents);
    persistIncidentLog(updatedIncidents);

    // Optionally raise an audit event (not displayed here)
    window.dispatchEvent(new CustomEvent("incident:created", { detail: newEntry }));
  ...
  const handleAddEvent = (e) => {
    ...
    const incident = incidentLog.find((i) => String(i.id) === String(newEvent.incidentId));
    if (!incident) return;
    ...
    const updatedTimeline = [...timelineEvents, entry];
  ...
  const handleAddRemediation = () => {
    ...
    const newTask = {
      id: remediationTasks.length ? remediationTasks[remediationTasks.length - 1].id + 1 : 1,
    ...
    const updatedTasks = [...remediationTasks, newTask];
    setRemediationTasks(updatedTasks);
  ...
  const showRemediation = ["Containment", "Eradication", "Recovery", "Lessons Learned"].includes(
    newEvent.phase
  );
    ...
    <div className="p-4 space-y-8">
      ...
      <div className="bg-white p-6 rounded-xl shadow-lg border-t-4 border-blue-500">
        ...
        <form onSubmit={handleAddEvent} className="space-y-4">
          ...
          {/* Logged By dropdown (dynamic roles) */}
          <div>
          ...
          {/* Remediation task (only for certain phases) */}
          {showRemediation && (
            <div>
...
const RemediationTasksPage = () => {
  ...
  // 🔹 Dynamic refresh when localStorage changes (e.g., IncidentBuilderPage adds a task)
  useEffect(() => {
    const handler = () => {
      try {
        const saved = localStorage.getItem("remediationTasks");
...
const AdminDashboardPage = () => {
  ...
  // Load roles from localStorage or fallback defaults
  const [users, setUsers] = useState(() => {
    try {
      const saved = localStorage.getItem("incidentUsers");
      return saved
  ...
  // Load default role from localStorage
  const [defaultRole, setDefaultRole] = useState(() => {
    try {
      return localStorage.getItem("defaultIncidentUser") || "SOC Analyst 1";
    } catch {
      return "SOC Analyst 1";
    }
  });
  ...
  const handleDeleteUser = (role) => {
    ...
    // If the deleted role was the default, reset default to first available
    if (role === defaultRole && updated.length > 0) {
      setDefaultRole(updated[0]);
      localStorage.setItem("defaultIncidentUser", updated[0]);
    }
  };

  const handleDefaultRoleChange = (e) => {
    const selected = e.target.value;
    setDefaultRole(selected);
    localStorage.setItem("defaultIncidentUser", selected);
  };
      ...
      <div className="bg-white p-6 rounded-xl shadow-lg border-t-4 border-green-500">
        ...
        <div className="mt-6">
          ...
          <p className="text-xs text-gray-500 mt-1">
            This role will be pre‑selected in IncidentBuilderPage.
          </p>
        </div>
      </div>
    </div>
